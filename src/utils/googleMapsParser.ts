...
async function decodePlusCode(
  ...
  try {
    ...
    // Perplexity AI для Plus Code - ОТКЛЮЧЕН
    // ПРИЧИНА: AI галлюцинирует и находит неправильные места
    // Например: Plus Code "WFX7+22W" в Mirissa AI декодирует как Colombo
    // РЕШЕНИЕ: Использовать только базу городов + Nominatim
    if (codeLength <= 8) {
      console.log(`⚠️ Короткий Plus Code (${codeLength} символов) - Perplexity AI отключен`);
      console.log(`💡 Используем базу городов + Nominatim для декодирования`);
      console.warn(`   Ожидаемая точность: ~100м - 5км в зависимости от длины кода`);
    }
    
    // ❌ ОТКЛЮЧЕНО - галлюцинирует координаты:
    // if (codeLength <= 8) {
