-- ================================================
-- 🔥 TELEGRAM LISTING SYSTEM - DATABASE SCHEMA (SAFE VERSION)
-- ================================================
-- Дата создания: 2026-01-27
-- Версия: SAFE - работает независимо от других таблиц
...
-- SAFE VERSION: UUID поля без foreign key constraints
-- ================================================
CREATE TABLE IF NOT EXISTS property_listings (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    
    -- Связь с клиентом (без FK для гибкости)
    client_request_id UUID, -- Ссылка на client_rental_requests (если есть)
    user_id UUID, -- Ссылка на users (если есть)
...
CREATE TABLE IF NOT EXISTS temperature_change_log (
    ...
    changed_by UUID, -- Ссылка на users (без FK)
...
DROP POLICY IF EXISTS "Admin full access to telegram_accounts" ON telegram_accounts;
...
DROP POLICY IF EXISTS "Admin full access to telegram_groups" ON telegram_groups;
...
DROP POLICY IF EXISTS "Users can read telegram_groups" ON telegram_groups;
CREATE POLICY "Users can read telegram_groups" ON telegram_groups
    FOR SELECT USING (is_active = true);

-- property_listings: пользователи видят только свои
DROP POLICY IF EXISTS "Users can manage own listings" ON property_listings;
...
DROP POLICY IF EXISTS "Users can read published listings" ON property_listings;
...
DROP POLICY IF EXISTS "All can read active publications" ON listing_publications;
CREATE POLICY "All can read active publications" ON listing_publications
    FOR SELECT USING (is_active = true);

DROP POLICY IF EXISTS "Admin full access to publications" ON listing_publications;
...
DROP POLICY IF EXISTS "Users can read responses to own listings" ON landlord_responses;
...
DROP POLICY IF EXISTS "Admin full access to responses" ON landlord_responses;
...
DROP POLICY IF EXISTS "All can read temperature log" ON temperature_change_log;
CREATE POLICY "All can read temperature log" ON temperature_change_log
    FOR SELECT USING (true);

DROP POLICY IF EXISTS "Admin can modify temperature log" ON temperature_change_log;
...
DROP TRIGGER IF EXISTS update_telegram_accounts_updated_at ON telegram_accounts;
CREATE TRIGGER update_telegram_accounts_updated_at BEFORE UPDATE ON telegram_accounts
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_telegram_groups_updated_at ON telegram_groups;
...
DROP TRIGGER IF EXISTS update_landlord_responses_updated_at ON landlord_responses;
...
-- ================================================
-- SAFE VERSION:
-- - Работает независимо от наличия таблиц users или client_rental_requests
-- - Использует IF NOT EXISTS для всех объектов
-- - Использует DROP IF EXISTS для политик и триггеров
