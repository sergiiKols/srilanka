...
-- Описание: Объекты недвижимости, которые арендаторы сохранили в свою коллекцию
-- ============================================
...
COMMENT ON COLUMN saved_properties.tenant_id IS 'ID арендатора (связь с таблицей tenants)';
COMMENT ON COLUMN saved_properties.telegram_user_id IS 'Telegram ID для быстрого поиска (дублируем для производительности)';
COMMENT ON COLUMN saved_properties.photos IS 'Массив URL фотографий объекта';
COMMENT ON COLUMN saved_properties.amenities IS 'Удобства в JSON формате: {wifi: true, pool: true, kitchen: true}';
COMMENT ON COLUMN saved_properties.source_type IS 'Источник: direct (прямая отправка) или forward (пересылка)';
COMMENT ON COLUMN saved_properties.forward_from_user_id IS 'ID автора оригинального сообщения (если было пересылка)';
COMMENT ON COLUMN saved_properties.original_message_link IS 'Ссылка на оригинальное сообщение в Telegram';
...
-- Триггер для обновления счётчика в tenants
